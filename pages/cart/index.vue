...
<div v-if="cart.length > 0">
  ...
  <div
    ...
    <div class="flex items-center gap-3">
      <button @click="decrementQuantity(item)" class="text-xl font-bold bg-blue-400 rounded-full text-white cursor-pointer">
       <Minus/>
      </button>
      <p>{{ item.quantity }}</p>
      <button @click="incrementQuantity(item)" class="text-xl font-bold bg-blue-400 rounded-full text-white cursor-pointer">
        <Plus/>
      </button>
    </div>
  ...
  <p class="text-3xl font-bold text-center pt-10">Корзина пуста</p>
</template>